...
function Navbar(props) {
  ...
  return (
    <div
      className=" bg- bg-indigo-700 flex justify-between p-10 lg:px-14
     md:flex  item-center"
    >
      ...
      <div className="flex justify-between -mt-[1rem]">
        <button
          onClick={downloadPDF}
          className="relative ml-3 z-0 inline-flex text-sm rounded-md shadow-sm  focus:z-10 focus:outline-none focus:ring-1"
        >
        ...
        <button className="relative ml-3 z-0 inline-flex text-sm rounded-md shadow-sm  focus:z-10 focus:outline-none focus:ring-1">
          <span className="relative inline-flex items-center px-3 py-3 space-x-2 text-sm font-medium text-azulScuro bg-white border border-gray-300 rounded-md sm:py-2">
            <Link href="cadastrar/transacao">
              <AiFillPlusCircle
                size={20}
                className="text-azulScuro right-5 cursor-pointer"
              />
            </Link>
          </span>
        </button>
